...
IMPORTANT: Only extract what is explicitly written. Never make assumptions."""
    ...
    def ai_powered_search(self, document_text: str, search_query: str) -> Dict[str, Any]:
        ...
        prompt = f"""
Analyze if this document is relevant to the search query: "{search_query}"
...
{{
    ...
    "matching_concepts": ["List concepts that match the search"],
    "relevant_excerpts": ["Up to 3 relevant quotes, max 50 words each"],
    "explanation": "Brief explanation of relevance",
    "confidence_score": 0.0
}}

Be precise. Only mark as relevant if there's a clear connection."""

        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a legal search specialist. Accurately assess document relevance."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.1,
                max_tokens=800,
    ...
    def ai_regulatory_tracking(self, document_text: str, regulation_topic: str) -> Dict[str, Any]:
        """
        Specialized AI analysis for regulatory tracking with broader matching
        """
        ...
        prompt = f"""
Analyze if this document relates to or mentions the regulation/topic: "{regulation_topic}"

Consider:
- Direct mentions of the regulation
- Related concepts and terminology
- Implementation or amendments
- Similar regulatory areas
...
{{
    ...
    "relationship_type": "Direct mention|Implementation|Amendment|Related topic|Reference",
    "specific_mentions": ["Direct quotes mentioning the topic"],
    "related_concepts": ["Related regulatory concepts found"],
    "temporal_references": ["Any dates or deadlines mentioned"],
    "evolution_indicators": ["How this document shows evolution of the topic"],
    "importance": "High|Medium|Low",
    "confidence_score": 0.0
}}

Be inclusive - include documents that are conceptually related, not just exact matches."""

        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a regulatory tracking expert. Find connections between documents and regulatory topics."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.15,  # Slightly higher for broader matching
                max_tokens=1000,
                response_format={"type": "json_object"}
            )
    ...
    def generate_ai_summary(self, document_text: str) -> Dict[str, Any]:
        """
        Generate concise AI summary for card view
        """
        ...
        prompt = f"""
Create a brief summary of this legal document.
...
{{
    "document_type": "Regulation|Directive|Decision|Other",
    "main_purpose": "1 sentence describing the main purpose",
    "key_points": [
        "First key point (max 15 words)",
        "Second key point (max 15 words)",
        "Third key point (max 15 words)"
    ],
    "importance": "High|Medium|Low",
...
Extract only what's explicitly stated."""
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a legal document summarizer. Be concise and accurate."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.1,
                max_tokens=500,
    ...
    def extract_document_structure(self, document_text: str) -> Dict[str, Any]:
        """
        AI extraction of document structure for table of contents
        """
        ...
        prompt = f"""
Extract the structure of this legal document.
...
{{
    ...
    "sections": [
        {{
            "type": "Article|Chapter|Section|Annex",
    ...
    "has_annexes": true/false,
    "total_sections": 0,
    "confidence_score": 0.0
}}

Only extract sections that are clearly marked in the text."""

        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a document structure analyst. Extract only explicit sections."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.05,
                max_tokens=1000,
            ...
            return {"sections": [], "confidence_score": 0.0}
    
    def assess_compliance_impact(self, document_text: str, sectors: List[str]) -> Dict[str, Any]:
        """
        AI assessment of compliance impact for specific sectors
        """
        ...
        prompt = f"""
Analyze compliance impact for these business sectors: {', '.join(sectors)}
...
{{
    ...
    "sector_impacts": {{
        "sector_name": {{
            ...
            "specific_requirements": ["List specific requirements"],
            "deadlines": ["Any deadlines found"],
            "actions_required": ["Required actions"]
        }}
    }},
    "cross_sector_requirements": ["Requirements affecting all sectors"],
...
Only include requirements explicitly stated in the document."""
        ...
        try:
            response = self.client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "You are a compliance expert. Extract only explicit requirements."},
                    {"role": "user", "content": prompt}
                ],
                temperature=0.1,
                max_tokens=1200,
            ...
            st.session_state.ai_analysis_cache[cache_key] = impact
            return impact
            
        except Exception as e:
            return {"overall_impact": "Error", "confidence_score": 0.0}
